...
if CFG.no_release:
    ...
    MODEL_PATH_2 =  os.path.join(curr_dir, "trained_models/6AAGCN_Focal_seqlen32_no_release_SAM_joints1_joints2_oridist/f10.8345518867924528_valloss234.12368774414062.pth") #_epoch12
else:
    MODEL_PATH_1 =  os.path.join(curr_dir, "trained_models/3_AAGCN_Focal_seqlen32_release_SAM_joints1_joints2_ori/f10.8439268867924529_valloss246.pth") #.87600708007812_epoch12
    MODEL_PATH_2 =  os.path.join(curr_dir, "trained_models/7_AAGCN_Focal_seqlen32_release_SAM_joints1_joints2_oridist/f10.8142688679245284_valloss31.pth") #0.2437744140625_epoch13

def train_eval():
    model_1.load_state_dict(torch.load(MODEL_PATH_1, map_location=torch.device('cpu'))["model_state_dict"])
    model_2.load_state_dict(torch.load(MODEL_PATH_2, map_location=torch.device('cpu'))["model_state_dict"])
