...
def train_func(model, data_loader, criterion, optimizer, scheduler, epoch, log_data):
    ...
    # Calculate accuracy
    accuracy_train = accuracy_score(np.concatenate(groundtruth), np.argmax(np.concatenate(preds), axis=1))

    log_data['epoch'].append(epoch)
    log_data['train_loss'].append(loss_total / iters)
    log_data['train_f1'].append(f1_train)
    log_data['train_accuracy'].append(accuracy_train)
...
def eval_func(model, criterion, data_loader, epoch, log_data):
    ...
    report = classification_report(
        groundtruth, preds, target_names=CFG.classes, labels=list(range(len(CFG.classes))), digits=3, output_dict=True
    )
    cm = confusion_matrix(groundtruth, preds, labels=list(range(len(CFG.classes))))
    ...
    # Append metrics to a single CSV
    metrics_df = pd.DataFrame(detailed_metrics)
    csv_file_path = f"{curr_dir}/trained_models/{CFG.experiment_name}/per_class_metrics.csv"
    if not os.path.exists(csv_file_path):
        metrics_df.to_csv(csv_file_path, index=False)
    else:
        metrics_df.to_csv(csv_file_path, mode='a', index=False, header=False)

    print(f"[INFO] Appended per-class metrics for epoch {epoch} to CSV")
    ...
    writer.add_scalar('Loss/Validation', loss_total / iters, epoch)
    writer.add_scalar('F1-Score/Validation/Micro', f1_val_micro, epoch)
    for metric in detailed_metrics:
        writer.add_scalar(f"Accuracy/Validation/{metric['class']}", metric['accuracy'], epoch)
...
train_loader = DataLoader(train_set, batch_size=CFG.batch_size, drop_last=True, shuffle=True,pin_memory=True) #collate_fn=custom_collate_fn
...
def train_eval():
    ...
    log_data = {'epoch': [], 'train_loss': [], 'train_f1': [], 'val_loss': [], 'train_accuracy': [],'val_f1_micro': [], 'classification_report': []}
    ...
    for epoch in range(start_epoch, CFG.epochs + start_epoch):
        ...
        #PATH = f"{curr_dir}/trained_models/{CFG.experiment_name}/{CFG.experiment_name}_epoch{epoch}.pth"
        torch.save({
            'epoch': epoch,
