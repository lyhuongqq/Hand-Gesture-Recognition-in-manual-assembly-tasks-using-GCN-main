...
def eval_func(model, criterion, data_loader, epoch, log_data):
    ...
    # Calculate accuracy eval
    accuracy_eval = accuracy_score(groundtruth, preds)
    ...
    log_data['accuracy_eval'].append(accuracy_eval)
    log_data['classification_report'].append(report)
    
    writer.add_scalar('Loss/Validation', loss_total / iters, global_step)
    # Log accuracy
    writer.add_scalar('Accuracy/Validation', accuracy_eval, global_step)

    # Print accuracy
    print(f"[EVAL] Accuracy_eval: {accuracy_eval}")
...
def train_eval():
    ...
    start_epoch = 0
    model.cuda()
    print(summary(model, (CFG.sequence_length, 21, CFG.num_feats),device="cuda")) #AGCN, STCONV
    ...
    log_data = {'epoch': [], 'train_loss': [], 'train_f1': [], 'val_loss': [],'train_accuracy': [], 'val_f1_micro': [], 'val_f1_macro': [],'accuracy_eval': [],'classification_report': []}
    ...
    for epoch in range(start_epoch, CFG.epochs + start_epoch):
        ...
        PATH = f"{curr_dir}/trained_models/{CFG.experiment_name}/AGCN_epoch{epoch}.pth"
        torch.save({
            'epoch': epoch,
