...
gt_data = pd.read_csv(os.path.join(curr_dir, "data/video_example/23.csv")) #, squeeze = True)
...
while cap.isOpened():
  ...
  landmarks_sliding_input = torch.from_numpy(landmarks_sliding[None, :, :]).cpu().float() #cuda
