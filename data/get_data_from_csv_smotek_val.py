...
def get_val_data():
    """Prepare validation data and balance it using SMOTETomek."""
    ...
    # Log validation distribution before balancing
    print_distribution(df_val, "INITIAL VALIDATION")

    # Apply SMOTETomek to balance the validation dataset
    df_val_balanced = apply_multiclass_smote_tomek(df_val)

    # Log validation distribution after balancing
    print_distribution(df_val_balanced, "BALANCED VALIDATION")

    # Export balanced validation data to JSON
    export_to_json(df_val_balanced, filename="val_data.json")
    return df_val_balanced
