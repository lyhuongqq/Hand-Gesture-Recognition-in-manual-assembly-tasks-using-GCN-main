...
def apply_smote_pipeline(df):
    """
    Apply SMOTE first, followed by Random_SMOTE sequentially.
    """
    df_numeric = preprocess_features(df)
    print("[INFO] Class distribution before any sampling:")
    print(df_numeric["LABEL"].value_counts())

    X = df_numeric.drop(columns=["LABEL"]).to_numpy()
    y = df_numeric["LABEL"].to_numpy()

    # Step 1: Apply SMOTE from imbalanced-learn
    smote = SMOTE(random_state=42) #sampling_strategy='auto', 
    X_smote, y_smote = smote.fit_resample(X, y)

    print("[INFO] Class distribution after applying SMOTE:")
    print(pd.Series(y_smote).value_counts())

    # Step 2: Apply Random_SMOTE from smote_variants
    random_smote = sv.Random_SMOTE(proportion=0.5, random_state=42)
    X_resampled, y_resampled = random_smote.sample(X_smote, y_smote)

    print("[INFO] Class distribution after applying Random_SMOTE:")
...
def get_train_val_data(all_ids, train_ratio=0.8):
    """Load all data, split into train/validation sets, and apply resampling using SMOTE and Random_SMOTE."""
    ...
    df_train_resampled = apply_smote_pipeline(df_train)

    print("[INFO] Validation LABEL distribution before resampling:")
    print(df_val["LABEL"].value_counts())
    df_val_resampled = apply_smote_pipeline(df_val)
