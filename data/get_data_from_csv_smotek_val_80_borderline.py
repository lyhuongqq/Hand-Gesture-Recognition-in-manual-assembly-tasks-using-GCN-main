...
def apply_borderline_smote(df, target_size=None):
    """
    Apply Borderline-SMOTE to balance all classes in the dataset.
    ...
    smote = BorderlineSMOTE(random_state=42, sampling_strategy=sampling_strategy, kind="borderline-1")
...
def get_train_val_data(all_ids, train_ratio=0.8):
    ...
    print("[INFO] Applying Borderline-SMOTE to the training set...")
    train_majority_size = df_train["LABEL"].value_counts().max()
    df_train_resampled = apply_borderline_smote(df_train, target_size=train_majority_size)

    # Resample the validation set
    print("[INFO] Applying Borderline-SMOTE to the validation set...")
    val_majority_size = df_val["LABEL"].value_counts().max()
    df_val_resampled = apply_borderline_smote(df_val, target_size=val_majority_size)
